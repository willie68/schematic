// Code generated by mockery; DO NOT EDIT.
// github.com/vektra/mockery
// template: testify

package internal

import (
	mock "github.com/stretchr/testify/mock"
)

// NewMockService creates a new instance of MockService. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewMockService(t interface {
	mock.TestingT
	Cleanup(func())
}) *MockService {
	mock := &MockService{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}

// MockService is an autogenerated mock type for the Service type
type MockService struct {
	mock.Mock
}

type MockService_Expecter struct {
	mock *mock.Mock
}

func (_m *MockService) EXPECT() *MockService_Expecter {
	return &MockService_Expecter{mock: &_m.Mock}
}

// Init provides a mock function for the type MockService
func (_mock *MockService) Init() error {
	ret := _mock.Called()

	if len(ret) == 0 {
		panic("no return value specified for Init")
	}

	var r0 error
	if returnFunc, ok := ret.Get(0).(func() error); ok {
		r0 = returnFunc()
	} else {
		r0 = ret.Error(0)
	}
	return r0
}

// MockService_Init_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Init'
type MockService_Init_Call struct {
	*mock.Call
}

// Init is a helper method to define mock.On call
func (_e *MockService_Expecter) Init() *MockService_Init_Call {
	return &MockService_Init_Call{Call: _e.mock.On("Init")}
}

func (_c *MockService_Init_Call) Run(run func()) *MockService_Init_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockService_Init_Call) Return(err error) *MockService_Init_Call {
	_c.Call.Return(err)
	return _c
}

func (_c *MockService_Init_Call) RunAndReturn(run func() error) *MockService_Init_Call {
	_c.Call.Return(run)
	return _c
}

// Shutdown provides a mock function for the type MockService
func (_mock *MockService) Shutdown() error {
	ret := _mock.Called()

	if len(ret) == 0 {
		panic("no return value specified for Shutdown")
	}

	var r0 error
	if returnFunc, ok := ret.Get(0).(func() error); ok {
		r0 = returnFunc()
	} else {
		r0 = ret.Error(0)
	}
	return r0
}

// MockService_Shutdown_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Shutdown'
type MockService_Shutdown_Call struct {
	*mock.Call
}

// Shutdown is a helper method to define mock.On call
func (_e *MockService_Expecter) Shutdown() *MockService_Shutdown_Call {
	return &MockService_Shutdown_Call{Call: _e.mock.On("Shutdown")}
}

func (_c *MockService_Shutdown_Call) Run(run func()) *MockService_Shutdown_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockService_Shutdown_Call) Return(err error) *MockService_Shutdown_Call {
	_c.Call.Return(err)
	return _c
}

func (_c *MockService_Shutdown_Call) RunAndReturn(run func() error) *MockService_Shutdown_Call {
	_c.Call.Return(run)
	return _c
}

// newMockhasher creates a new instance of mockhasher. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func newMockhasher(t interface {
	mock.TestingT
	Cleanup(func())
}) *mockhasher {
	mock := &mockhasher{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}

// mockhasher is an autogenerated mock type for the hasher type
type mockhasher struct {
	mock.Mock
}

type mockhasher_Expecter struct {
	mock *mock.Mock
}

func (_m *mockhasher) EXPECT() *mockhasher_Expecter {
	return &mockhasher_Expecter{mock: &_m.Mock}
}

// RebuildAllHashes provides a mock function for the type mockhasher
func (_mock *mockhasher) RebuildAllHashes() error {
	ret := _mock.Called()

	if len(ret) == 0 {
		panic("no return value specified for RebuildAllHashes")
	}

	var r0 error
	if returnFunc, ok := ret.Get(0).(func() error); ok {
		r0 = returnFunc()
	} else {
		r0 = ret.Error(0)
	}
	return r0
}

// mockhasher_RebuildAllHashes_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'RebuildAllHashes'
type mockhasher_RebuildAllHashes_Call struct {
	*mock.Call
}

// RebuildAllHashes is a helper method to define mock.On call
func (_e *mockhasher_Expecter) RebuildAllHashes() *mockhasher_RebuildAllHashes_Call {
	return &mockhasher_RebuildAllHashes_Call{Call: _e.mock.On("RebuildAllHashes")}
}

func (_c *mockhasher_RebuildAllHashes_Call) Run(run func()) *mockhasher_RebuildAllHashes_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *mockhasher_RebuildAllHashes_Call) Return(err error) *mockhasher_RebuildAllHashes_Call {
	_c.Call.Return(err)
	return _c
}

func (_c *mockhasher_RebuildAllHashes_Call) RunAndReturn(run func() error) *mockhasher_RebuildAllHashes_Call {
	_c.Call.Return(run)
	return _c
}
